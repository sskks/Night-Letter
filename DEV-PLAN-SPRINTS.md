...
| A3 | 云端同步无去重 | S2 | `syncDreamsToCloud()` L1611 | 每次登录都执行 `col.add()` 逐条添加，没有检查云端是否已存在相同 `localId`，重复登录会产生大量重复文档 | ✅ Sprint 2 已修复 (47da3ed) |
| A4 | 合并策略可能丢数据 | S2 | `mergeDreams()` L1644 | `map[d.id]=d` 后者覆盖前者。如果本地数据更新但云端未同步，登录后云端旧数据会覆盖本地新数据 | ✅ Sprint 2 已修复 (47da3ed) |
| A5 | 注册后未自动登录(v1) | S2 | `doRegister()` L1484 | v1 SDK 的 `signUpWithEmailAndPassword` 不会自动登录，代码虽然有 `_cbSignIn` 补登录逻辑，但如果 signIn 也失败（如网络波动），用户会卡在"注册成功但未登录"的状态 | ✅ Sprint 2 已修复 (47da3ed) |
